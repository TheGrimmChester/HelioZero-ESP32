...
bool config_apply_from_json(JsonObject root, bool fullPut, String &err) {
  const char *keys[] = {"dhcp_on", "ip_fixed", "gateway", "subnet_mask", "dns", "source", "peer_ip",
  ...
  if (root.containsKey("peer_ip")) {
    if (!applyIp("peer_ip", peer_ip)) return false;
  }
  if (root.containsKey("peer_port")) {
    int p = (int)root["peer_port"];
    if (p < 1 || p > 65535) {
      err = "peer_port must be 1..65535";
      return false;
    }
    peer_port = (unsigned int)p;
  }
  if (root.containsKey("peer_path")) {
    String path = root["peer_path"].as<String>();
    path.trim();
    if (path.length() == 0 || path[0] != '/') {
      err = "peer_path must be non-empty and start with /";
      return false;
    }
    if (path.indexOf("..") >= 0) {
      err = "peer_path invalid";
      return false;
    }
    if (path.length() > 48) {
      err = "peer_path too long (max 48)";
      return false;
    }
    peer_path = path;
  }
  if (root.containsKey("peer_protocol")) {
    String proto = root["peer_protocol"].as<String>();
    proto.trim();
    proto.toLowerCase();
    if (proto != "json") {
      err = "peer_protocol must be json";
      return false;
    }
    peer_protocol = "json";
  }
  ...
  if (root.containsKey("jsy_mk333_serial_baud")) JsyMk333SerialBaud = (uint32_t)root["jsy_mk333_serial_baud"].as<unsigned long>();
