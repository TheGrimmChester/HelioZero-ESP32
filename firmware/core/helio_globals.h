...
// --- HelioPeer (remote router) — User: GUIDE A.6.1; See: /en/hardware-pinout/ source_heliopeer ---
extern unsigned long peer_ip;
/** TCP port for HelioPeer remote HTTP (default 80). REST: peer_port */
extern unsigned int peer_port;
/** HTTP path for HelioPeer snapshot, e.g. /api/v1/measurements (max 48 chars). */
extern String peer_path;
/** HelioPeer HTTP protocol (always JSON snapshot). */
extern String peer_protocol;
extern unsigned long peer_last_poll_ms;
extern bool peer_last_poll_ok;
extern String peer_last_poll_err;
extern String peer_last_poll_preview;
extern String peer_last_poll_protocol;
...
/** HTTP path for HelioPeer main snapshot (default /api/v1/measurements). */
...
// --- Analog calibration (ADC) — See: /en/hardware-pinout/ source_analog ---
...
/** When true, force triac off while source health is stale (split HelioPeer installs). */
...
extern int WIFIbug;
/** Failed HTTP to external meter peers (Shelly, HelioPeer, …); logged only — does not affect Wi‑Fi. */
