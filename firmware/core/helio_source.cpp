...
extern void analog_probe_setup(void);
...
extern void analog_probe_poll(void);
...
/** base_period_ms from helio_source_logic_base_poll_period_ms (JsyMk333 baud-dependent). */
static const HelioSourceRow kRegistry[] = {
    {SourceId::JsyMk194, "JsyMk194", jsy_mk194t_setup, jsy_mk194t_poll, 400, RSF_NONE},
    {SourceId::JsyMk333, "JsyMk333", jsy_mk333_setup, jsy_mk333_poll, 800, RSF_NONE},
    {SourceId::Analog, "Analog", analog_probe_setup, analog_probe_poll, 40, RSF_NONE},
    ...
    {SourceId::HelioPeer, "HelioPeer", nullptr, external_peer_poll, 800, (uint8_t)(RSF_POLL_BACKOFF | RSF_TOUCH_LAST_MS)},
};
...
void helio_source_apply_hardware_setup() {
  ...
  if (g_active_source == SourceId::JsyMk333) {
    ...
    poll_period_ms = helio_source_logic_base_poll_period_ms(SourceId::JsyMk333, JsyMk333SerialBaud);
  }
  if (g_active_source != SourceId::HelioPeer) {
    Source_data = Source;
  }
}
...
void helio_source_run_poll_cycle(unsigned long pollBackoffMs) {
  ...
  uint32_t p = helio_source_logic_base_poll_period_ms(g_active_source, JsyMk333SerialBaud);
...
void helio_sources_diagnostics_append_summary(JsonObject root) {
  ...
  if (active == SourceId::HelioPeer && Source_data.length() > 0) {
  ...
  if (active == SourceId::HelioPeer) {
    diag["last_poll_ok"] = peer_last_poll_ok;
    diag["last_error"] = peer_last_poll_err;
    diag["protocol_used"] = peer_last_poll_protocol.length() ? peer_last_poll_protocol : "json";
    diag["peer_protocol"] = peer_protocol.length() ? peer_protocol : "json";
    if (peer_last_poll_ms > 0) {
      diag["transport_last_poll_ms_ago"] =
          (int)((millis() - peer_last_poll_ms) & 0x7FFFFFFF);
    } else {
  ...
  if (helio_diag_analog_adc_clipping_active()) {
...
void helio_sources_diagnostics_append_meter_payload(JsonObject doc, int linky_tail_max) {
  ...
  if (eff == SourceId::Analog || eff == SourceId::JsyMk194) {
    JsonObject wf = doc.createNestedObject("analog_waveform");
  ...
  if (helio_active_source_get() == SourceId::HelioPeer) {
    JsonObject peer = doc.createNestedObject("peer");
    peer["peer_ip"] = ip32ToDotted(peer_ip);
    peer["peer_port"] = peer_port;
    peer["peer_path"] = peer_path;
    peer["peer_protocol"] = peer_protocol.length() ? peer_protocol : "json";
    peer["last_poll_ok"] = peer_last_poll_ok;
    peer["last_poll_ms_ago"] =
        (peer_last_poll_ms > 0) ? (int)((millis() - peer_last_poll_ms) & 0x7FFFFFFF) : -1;
    peer["last_error"] = peer_last_poll_err;
    peer["last_frame_preview"] = peer_last_poll_preview;
    peer["protocol_used"] = peer_last_poll_protocol.length() ? peer_last_poll_protocol : "json";
  }
}
