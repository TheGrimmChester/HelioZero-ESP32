...
static const SourceWireRow kWireTable[] = {
    {SourceId::JsyMk194, "JsyMk194"},   {SourceId::JsyMk333, "JsyMk333"},   {SourceId::Analog, "Analog"},
    ...
    {SourceId::Pmqtt, "Pmqtt"},   {SourceId::NotDef, "NotDef"},   {SourceId::HelioPeer, "HelioPeer"},
};
...
SourceId helio_source_logic_effective_id(SourceId active, const char *source_data_wire) {
  if (active == SourceId::HelioPeer) return helio_source_logic_parse_wire(source_data_wire);
  return active;
}
...
uint16_t helio_source_logic_base_poll_period_ms(SourceId id, uint32_t jsy_mk333_serial_baud) {
  switch (id) {
