...
struct EepromExtensionFields {
  ...
  /** HelioPeer HTTP port (default 80; 0 coerced to 80). */

  uint16_t peer_port = 80;

  /** HelioPeer snapshot path (default "/api/v1/measurements", max 48 chars). */

  std::string peer_path = "/api/v1/measurements";

  /** HelioPeer HTTP wire format: 1 = JSON (/api/v1/measurements). */

  uint8_t peer_protocol_mode = 1;
