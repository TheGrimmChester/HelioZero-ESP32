...
TEST(FleetBundleLogic, RejectsForbiddenSecrets) {
  ...
  EXPECT_FALSE(fleet_bundle_contains_forbidden_secret("{\"source\":\"HelioPeer\"}"));
}
...
TEST(FleetBundleLogic, ForbiddenSecretVariants) {
  ...
  EXPECT_TRUE(fleet_bundle_contains_forbidden_secret(R"({"api_access_token":"x"})"));
...
TEST(FleetBundleLogic, VerifyWrongHexSameLength) {
  const std::string sig = fleet_bundle_sign_hmac_sha256_hex("p", "k");
  ASSERT_FALSE(sig.empty());
  std::string bad = sig;
  bad[0] = (bad[0] == 'a' ? 'b' : 'a');
  EXPECT_FALSE(fleet_bundle_verify_hmac_sha256_hex("p", "k", bad));
}

TEST(FleetBundleLogic, SafePayloadWithoutSecrets) {
  EXPECT_FALSE(fleet_bundle_contains_forbidden_secret("{}"));
  EXPECT_FALSE(fleet_bundle_contains_forbidden_secret(""));
}
