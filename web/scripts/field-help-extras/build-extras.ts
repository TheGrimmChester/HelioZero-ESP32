...
function settingsExtras(lang: DocsLang): Record<string, readonly string[]> {
  ...
  return {
    ...
    triac_off_when_source_stale: [
      h,
      lang === "fr"
        ? "Recommandé `true` pour `Ext` / compteurs HTTP : ex. couper si aucune mesure < 30 s."
        : "Recommended `true` for `Ext` / HTTP meters: e.g. off when no fresh sample for ~30 s.",
    ],
    triac_backoff_when_heater_idle: [
      h,
      lang === "fr"
        ? "UxIx2 + CH2 sur la charge routée : triac coupé si P consigne haute mais CH2 ≈ 0 W ~45 s."
        : "UxIx2 + CH2 on routed branch: triac off if commanded high but CH2 ≈ 0 W for ~45 s.",
    ],
...
function sourceWizardExtras(lang: DocsLang): Record<string, readonly string[]> {
  ...
  return {
    ...
    pmqtt_topic: pmqttTopicBlocks(lang),
    uxix3_serial_baud: settingsExtras(lang).uxix3_serial_baud,
    calib_u: calibBlocks(lang),
    calib_i: calibBlocks(lang),
    pmqtt_preset: pmqttPresetBlocks(lang),
    defer_test: [
