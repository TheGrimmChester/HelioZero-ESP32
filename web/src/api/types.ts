...
export interface RouterConfig {
  ...
  peer_port: number;
  peer_path: string;
  /** HelioPeer HTTP wire format (JSON `/api/v1/measurements` only on firmware 0.3+). */
  peer_protocol?: "json";
  enphase_user: string;
  ...
  /** Débit série Serial2 pour JSY-MK-333 (JsyMk333). */
...
export interface SourceDiagnostics {
  ...
  current_second_a: number;
  pf_second: number;
  analog_waveform?: { volt_m: number[]; amp_m: number[] };
  linky?: {
    ltarf: string;
    idx_raw: number;
    tail: string;
