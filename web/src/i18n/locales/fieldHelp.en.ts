...
export const fieldHelpEn = {
  settings: {
    ...
    triac_off_when_source_stale:
      "Forces triac off when measurement data is too old or invalid. Strongly recommended for remote sources (HelioPeer, HTTP meters) to avoid blind heating.",
    triac_backoff_when_heater_idle:
      "With JsyMk194 (or similar) and CH2 wired on the routed load: turns triac off if commanded power is high but CH2 shows no load for ~45 s (series thermostat open). Wire CH2 on the routed load branch (after the triac), not another circuit.",
    max_routed_w:
    ...
    calib_u:
      "Scales measured voltage for Analog analog inputs. Factory default 1000; increase if readings are low vs a reference meter.",
    calib_i:
      "Scales measured current for Analog analog inputs. Factory default 1000; adjust after clamp installation and reference comparison.",
    pmqtt_topic:
    ...
    jsy_mk333_serial_baud:
      "UART baud rate for JSY-MK-333 (JsyMk333) on UART2. Default 9600; must match meter configuration (1200–115200).",
    pwm_gpio:
  ...
  sourceWizard: {
    ...
    peer_port: "TCP port (default 80). Use if the peer listens on a non-standard port or behind a reverse proxy.",
    peer_path:
