...
export const fieldHelpFr = {
  settings: {
    ...
    triac_off_when_source_stale:
      "Coupe le triac si les mesures sont trop anciennes ou invalides. Recommandé pour sources distantes (HelioPeer, HTTP).",
    triac_backoff_when_heater_idle:
      "Avec JsyMk194 et CH2 sur la charge routée : coupe le triac si commande active mais CH2 ≈ 0 pendant ~45 s (thermostat en série ouvert). Câbler CH2 sur la branche de la charge routée (après le triac).",
    max_routed_w:
    ...
    calib_u:
      "Coefficient de tension pour entrées analogiques Analog. Défaut 1000 ; augmenter si la lecture est basse vs référence.",
    calib_i:
      "Coefficient de courant Analog. Défaut 1000 ; ajuster après pose des TC et comparaison compteur.",
    pmqtt_topic:
    ...
    jsy_mk333_serial_baud:
      "Débit UART pour JSY-MK-333 (JsyMk333) sur UART2. Défaut 9600 ; doit correspondre au compteur (1200–115200).",
    pwm_gpio:
  ...
  sourceWizard: {
    ...
    peer_port: "Port TCP (80 par défaut). Si écoute non standard ou proxy.",
    peer_path:
