...
function extProtocolLabel(_proto: string | undefined, T: AppStrings): string {
  return T.sourceWizard.peerProtocolJson;
}
...
function connectionStatus(
  ...
  const pollOk =
    diag?.last_poll_ok ??
    (d?.peer != null ? d.peer.last_poll_ok : undefined);
  const ok = !!(pollOk && d?.date_valid);
  const ms =
    diag?.last_poll_ms_ago ??
    d?.peer?.last_poll_ms_ago;
  const pollSuffix =
    ms != null && ms >= 0
  ...
  const err =
    (diag?.last_error && diag.last_error.length > 0
      ? diag.last_error
      : undefined) ||
    (d?.peer?.last_error && d.peer.last_error.length > 0
      ? d.peer.last_error
      : undefined);
  if (err) {
...
function peerUrlFromConfig(cfg: RouterConfig): string {
  const ip = formatPeerIp(cfg.peer_ip);
  if (ip === "—") return "—";
  const port = cfg.peer_port || 80;
  const path = (cfg.peer_path || "").trim() || "/";
  const p = path.startsWith("/") ? path : `/${path}`;
  return `${ip}:${port}${p}`;
}

function peerUrlFromPeer(d: SourceDiagnostics): string {
  const ext = d.peer!;
  const ip = formatPeerIp(ext.peer_ip);
  if (ip === "—") return "—";
  let port = 80;
  if (ext.peer_port) port = ext.peer_port;
  const pathTrim = (ext.peer_path ?? "").trim();
  let p = "/";
...
export function buildSourceSummaryRows(
  ...
  if (sourceKey === "heliopeer" || wire === "HelioPeer") {
    const peer = d?.peer ? peerUrlFromPeer(d) : peerUrlFromConfig(cfg);
    rows.push({ label: SS.peerUrl, value: peer });
    const protoCfg =
      d?.diagnostics?.peer_protocol ||
      d?.peer?.peer_protocol ||
      cfg.peer_protocol;
    rows.push({
    ...
    const used =
      d?.diagnostics?.protocol_used || d?.peer?.protocol_used;
    if (used) {
      rows.push({
  ...
  if (lanSources.has(sourceKey)) {
    const ip =
      formatPeerIp(cfg.peer_ip) !== "—"
        ? formatPeerIp(cfg.peer_ip)
        : formatPeerIp(d?.peer?.peer_ip);
